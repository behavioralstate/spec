...
0.9.17 lets the person name the connection. Measured with 0.9.15's words: the assistant kept the credential under a name of its own, and the credential block's `mcp` entry was keyed by the host, so the name the person used in the prompt ("remundo dev xml") reached nothing. And two doors of one platform — a customer surface and a back office, each with its own credential and its own permissions, for the same person — need two names the person recognises. The guidance now says to make one word of the person's name for the service, confirm it with them, and send it as `connection`; the token answer gives it back as `name`, keys the `mcp` entry by it, and the credential is kept under it. The service stores the word and reads nothing into it: it is the consumer's label, never identity.

0.9.18 says what a leak is. Measured with 0.9.17's words: the assistant saved the token answer to a file, read the file back to inspect it, saw the credential in its own output, and — following "a credential that appears there is leaked and must be replaced" — opened a new registration and asked the person to approve again, four seconds after the key was issued; and again after that, without end. The token text now says that receiving the answer, or reading it back from where it was saved, is not a leak; that a credential is leaked only when it has been shown to the person or written somewhere else; that a leaked credential is the person's to revoke on the service and re-issue; and that the agent never starts another sign-in on its own for a credential it already holds.
