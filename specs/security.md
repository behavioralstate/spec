...
`GET /.well-known/best` is always unauthenticated — it is the public discovery endpoint. Beyond it, only two things are reachable without a BEST credential:

- the capabilities of a service whose manifest entry declares `"authentication": { "type": "none" }` — a **public surface**; and
- the `deviceAuthorizationUrl` and `tokenUrl` endpoints of [agent registration](identity-and-registration.md), which **MUST** be rate-limited per client and per `device_code`.

Every other endpoint **MUST** require authentication. A public surface **MUST NOT** return data belonging to any account and **MUST** be rate-limited per client. A command accepted on a public surface **MUST** have no effect on any account until a person signed in to the platform has approved it.
...
- Commands that are destructive, irreversible, that bypass a compliance control, that commit a person (open an account, accept terms, start a subscription) or that grant access to what is theirs (issue a credential, share a resource, invite a member) **SHOULD** require a control beyond the submitting credential: human approval, a second principal, or an out-of-band confirmation.
- On a public surface there is no submitting credential at all, so the control is the whole of the authorisation: the person signing in to the platform and approving, in their own browser, is the out-of-band confirmation. It is not self-serviceable by construction — the agent cannot perform it.
...
- A value that names something — an envelope `id`, a `correlationid`, a tenant, account or registration identifier — **MUST NEVER** be accepted as proof of anything. Identifiers are logged, projected and indexed by design.
- Every secret a service relies on **MUST** be generated by the service, never minted by the caller. A caller may be a model, and a model cannot produce randomness: it repeats the example values it has seen.
- This is why [agent registration](identity-and-registration.md#why-not-a-best-command) is the RFC 8628 exchange and not a command: its `device_code` is service-generated and handed over synchronously, and is neither derived from nor usable as any identifier.

## Credentials Stay Out of Transcripts

A conversation with a model is not a secret store: it is kept, synced, summarised and shared.
...
- A service that declares `deviceAuthorizationUrl` **MUST** carry the [sign-in guidance](https://github.com/behavioralstate/spec/blob/main/SPEC.md#sign-in-guidance) verbatim at each step — the manifest text as that `authentication` block's `note`, the device text as the device answer's `note`, and the token text at the start of the token answer's `note` — because the consumer is often a model with nothing else to tell it. The texts say that the manifest is for the agent, that the person approves in their own browser, and that the credential is saved where only the person can read it, under the connection's name, and never appears in a message, a command or any output.
- An agent **MUST NOT** open the verification link itself or in a browser it controls: it gives the link to the person, who approves in their own browser. A page the agent drives could approve on the person's behalf, or put their password within its reach.
- The token answer **SHOULD** offer the ways to use the credential: `mcp`, the connection as an MCP client's configuration, and `http`, direct calls with a placeholder in place of the credential ([Agent Registration](https://github.com/behavioralstate/spec/blob/main/SPEC.md#agent-registration), the credential block). The consumer chooses. No other service text instructs the consumer about its client ([Manifest Discipline](https://github.com/behavioralstate/spec/blob/main/SPEC.md#manifest-discipline) rule 5).
...
- A tenant ID names a real scope. A constant pseudo-tenant (`public`, `default`, `anonymous`) hosting a surface that has no scope is non-conformant from 0.10.0 — declare a root-level service with its own `authentication` instead.
