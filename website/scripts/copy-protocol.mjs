 ...
 * occurrence of the literal string {{BEST_VERSION}} in the copied files with
...
/** Recursively copy src → dest, stamping {{BEST_VERSION}} in every text file. */
function copyAndStamp(src, dest) {
  ...
  for (const entry of readdirSync(src, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      const stamped = content.replaceAll('{{BEST_VERSION}}', version);
