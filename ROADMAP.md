...
1. **`best-validate` conformance CLI** — ✅ built ([validate-cli/](validate-cli/)): executable conformance checklist with schema validation, non-destructive probes, multi-tenant support, `--json` for CI, and a temporary `--legacy-bsp` mode for pre-0.9.0 endpoints (remove once dotquant/remundo migrate). Published as [`@behavioralstate/best-validate`](https://www.npmjs.com/package/@behavioralstate/best-validate) 0.1.0 (`validate/v*` tags, `./scripts/release-validate.sh`). Remaining: run it against both live endpoints.
