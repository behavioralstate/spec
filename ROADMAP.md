...
1. **`best-validate` conformance CLI** — an executable checker for live endpoints (fetch manifest → verify capability declarations → probe required endpoints → validate shapes against the schemas). The single biggest enabler for credible third-party implementations, which are in turn the gate to full standards-track.
