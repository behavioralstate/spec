...
Last updated: 2026-09-19 (0.9.10 released — name resolution: a DNS name is the whole namespace, with obligations on the resolving consumer; no wire change).
...
- [ ] **Name resolution in `best-mcp`** — SPEC.md *Name Resolution* (0.9.10) makes a DNS name the whole namespace; the client is the missing resolver. Today a connection exists only where `BEST_<APP>_BASE_URL` was configured, per service and per machine — which is why every service ends up shipping its own client snippet. Design: a `connect` tool taking a `name`; it runs the resolution algorithm, **returns what it resolved (name, canonical endpoint host, the manifest's service descriptions) without connecting**, and opens the connections only on a second call carrying the person's confirmation — so the confirmation is a tool-level step a model cannot skip, not a sentence in the instructions. Resolved connections are named `<name>/<serviceId>` and live beside configured ones; the credential store is already keyed by base URL, which is the name binding the spec requires. Refuse IP literals, private/loopback targets and non-`https`; `TXT` lookup over the system resolver, skipped where unavailable. No service-specific text enters the client: it learns how to look a service up, never what a service does. Open questions before building: whether the `TXT` step earns its complexity (the well-known path plus Origin Discovery's redirect already covers an API on another host; the record only adds names with no HTTP origin at all); whether resolved connections persist across sessions or only their credentials do; how `best-validate` checks that a name resolves.
