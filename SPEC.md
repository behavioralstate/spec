...
**What a site shows the person.** A site's own "connect your assistant" page **may** give the person the sentence `Sign me in to <name>`, where `<name>` is a name the service is publicly known by. It **must not** append URLs, tenant IDs or steps: the name is the whole instruction ([Name Resolution](#name-resolution)), and anything added is a second agent-directed description. A name that does not resolve yet is fixed with the bridge above, never with a longer sentence.
...
**Errors.** `deviceAuthorizationUrl` and `tokenUrl` answer errors per [RFC 6749 §5.2](https://www.rfc-editor.org/rfc/rfc6749#section-5.2), not in the BEST error format: status `400` (or `401` for a client failure) with a JSON body carrying `error` and, **recommended**, `error_description`. A request that is not form-encoded is `invalid_request` — never a bare status with an empty body, because the agent is often a model that corrects itself only from what the answer says. `tokenUrl` uses the RFC 8628 §3.5 codes above.
