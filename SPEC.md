...
- **Bind every credential to the name it was issued under.** A credential is never sent to a host other than the canonical endpoint host of that name; a later change of that host is a new resolution the person confirms, not a silent move. It is kept under that name too — the `connection` the agent sends when it signs the person in ([Agent Registration](#agent-registration)), which the token answer gives back as `name`: another name is another sign-in, kept apart, with its own credential and its own permissions, and a configuration entry made from the credential takes the same name.
...
A service that lets an agent obtain its own credential declares `deviceAuthorizationUrl` and `tokenUrl`. The exchange is [RFC 8628](https://www.rfc-editor.org/rfc/rfc8628), unchanged; BEST adds three request parameters (`connection`, `agent_label`, `credential_lifetime`), three members of the token answer (`name`, `tenant_id`, `auth_header`), the [credential block](#agent-registration) that comes with the credential, and the [sign-in guidance](#sign-in-guidance): the words, fixed by this specification, that tell the consumer at each step what to do.
...
| `connection` | no | The name of this connection: **one word** of lowercase letters, digits and hyphens (`^[a-z0-9][a-z0-9-]{0,63}$`), made from the name the person gave the service — "acme payroll dev" becomes `acme-payroll-dev`; its host when they gave none — and confirmed with them before the request. The same person signing in to two doors of one platform (a customer surface and a back office, each with its own credential and its own permissions) gives two names. The service keeps it with the registration and gives it back as `name` in the token answer, unread: it is the consumer's label, never identity. Another shape is `invalid_request`, with an `error_description` that states the shape. |
...
**4. On approval the token endpoint answers** per RFC 6749 §5.1 — `access_token`, `token_type`, and `expires_in` when the credential is not durable — with three BEST members: `name`, the connection's name — the `connection` the agent sent, or the address of the sign-in manifest when it sent none; `tenant_id`, the account the credential opens; and `auth_header`, the header that carries it when that is not `Authorization`. A `device_code` is redeemed once.
...
| `name` | yes | The connection's name: the `connection` the agent sent to `deviceAuthorizationUrl`, else the address of the sign-in manifest (its host, and its path when it is not at the root). The credential is kept under it, and the `mcp` entry is keyed by it |
...
| `mcp` | no | This connection as an MCP client's configuration (for the reference MCP server: `command`, `args`, `env`) — the credential in its `env` — as **one entry keyed by `name`**. Two sign-ins are two names the person gives ([Name Resolution](#name-resolution)), so two entries: neither overwrites the other |
...
> The credential is a secret. Keep it in your client's configuration or credential store, under the name in this answer, the connection the person named: another name is another sign-in, kept apart, with its own credential and its own permissions. Receiving this answer is not a leak, and neither is reading it back from where you saved it: save it as it arrived, then work from there and never repeat the credential in a message, a command or any output. It is leaked only if you have shown it to the person or written it somewhere else; then say so and ask them to revoke it on the service and sign in again. Never start another sign-in on your own for a credential you already hold. If your client cannot store it, say so instead of showing it.
...
6. **The tenants root says where a credential belongs.** A service the root manifest announces for its tenants ([Manifest Discipline](#manifest-discipline) rule 3) may give it, in the root manifest, the `http.endpoint` that each tenant manifest's `http.endpoint` for the same service extends with `/{tenantId}` — its **tenants root** (`https://api.example.com/api/best/tenants` for the `…/tenants/acme` of rule 2). A consumer finds it as that service's `http.endpoint` in the root manifest. A `GET` on a tenants root **should** answer, with a credential, `200` and the tenants that credential opens: `{ "tenants": [ { "tenant_id", "endpoint", "manifest" } ] }`. Without one it answers the `401` a tenant endpoint gives, naming the way in; a service that cannot tell which tenant a credential opens answers a BEST error that says where the consumer finds it — the `endpoint` and `tenant_id` of the token answer, or a new sign-in. **Never a bare `404`**: a consumer that kept only its credential — a model after a restart — otherwise concludes it has no account.
