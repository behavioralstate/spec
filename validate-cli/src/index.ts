...
async function checkWorkflows(cap: Capability, ep: string, ctx: ProbeContext, opts: Options, ajv: Ajv2020): Promise<void> {
  ...
  const minting: string[] = [];
  // A validator is a guest: each schema document is fetched once however many recipes name it, and a service that
  // answers 429 has said "not now", not "not there".
  ...
  for (const w of index.slice(0, 8)) {
    ...
    for (const step of steps) {
      ...
      if (new URL(ds).origin === new URL(ep).origin && !throttled) {
        if (!fetched.has(ds)) fetched.set(ds, (await http('GET', ds, opts, auth)).status);
        const status = fetched.get(ds)!;
        if (status === 429) { throttled = true; continue; }
        if (status !== 200 && !(status === 401 && !auth)) unresolved.push(`${id}: ${ds} → ${status || 'unreachable'}`);
      }
    }
  ...
  else if (throttled) record(S, 'skip', `The service rate-limited the recipe walk after ${[...fetched.values()].filter(st => st !== 429).length} schema documents — the steps checked so far resolve; run again later for the rest`);
